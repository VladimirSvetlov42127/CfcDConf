...
CinModel::CinModel(const std::vector<CinSignal*>& items, QObject* parent)
    : QAbstractTableModel{parent}
    , m_items{items}
{
}

int CinModel::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid())
        return 0;
...
int CinModel::columnCount(const QModelIndex &parent) const
{
    if (parent.isValid())
        return 0;
...
QVariant CinModel::headerData(int section, Qt::Orientation orientation, int role) const
{
...
Qt::ItemFlags CinModel::flags(const QModelIndex &index) const
{
...
QVariant CinModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid()) {
        return QVariant();
    }
    ...
    if (role == Qt::EditRole && column == CinModel::Column::Name) {
        if (item->subtype() & (Signal::Subtype::Virtual | Signal::Subtype::Remote))
...
bool CinModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    if (!index.isValid())
        return false;
