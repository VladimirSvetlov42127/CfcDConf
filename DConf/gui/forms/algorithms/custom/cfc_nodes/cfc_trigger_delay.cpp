...
CfcTriggerDelay::CfcTriggerDelay(QString id, QSizeF node_size, QGraphicsItem* parent) : CfcNode(id, parent)
{
...
CfcTriggerDelay::CfcTriggerDelay(QDomNode xml, QGraphicsItem* parent) : CfcNode(xml, parent)
{
    //  Настройка параметров
    setName("TriggerDelay");
    setNodeType(RZA_DELAY_ON);
...
CfcTriggerDelay::CfcTriggerDelay(RamNode node, QGraphicsItem* parent) : CfcNode(node, parent)
{
    //  Настройка параметров
    setName("TriggerDelay");
    setNodeType(RZA_DELAY_ON);
...
void CfcTriggerDelay::paintElement(QPainter* painter)
{
    ...
    painter->drawText(QPointF(rectangle.left() + 2 * margin_w, 2 * margin_h), "t");
