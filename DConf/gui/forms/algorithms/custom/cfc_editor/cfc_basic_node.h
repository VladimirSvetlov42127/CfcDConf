...
//  Структура для копирования элемента
struct  RamNode {
    QString name;                   //  Тип элемента
    QPointF position;               //  Расположение элемента
    QSizeF size;                    //  Размер элемента
    uint8_t outputs;                //  Количество выходов
    uint8_t inputs;                 //  Количество входов
    QList<NodeParam> params_list;   //  Список параметров элнмента

};

//	список переменных для компиляции
namespace {
...
public:
    ...
    CfcBasicNode(RamNode node, QGraphicsItem* parent = nullptr);
